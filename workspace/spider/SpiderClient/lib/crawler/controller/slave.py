...
class Subordinate(object):
    def __init__(self, host, port, main_host, workers, env, recv_real_time_request=False):
        '''
            初始化
        ...
        # 保存subordinate的相关信息
        self.info = SubordinateInfo(local_ip=host, port=port,
    ...
    def register_in_main(self):
        ...
        try:
            logger.info('subordinate register to host:{0}'.format(
                self.__client.host))
            res = self.__client.get(path).strip()
            logger.info('subordinate register to host:{0} res:{1}'.format(
                self.__client.host, res))
            if json.loads(res)['error']['error_id'] != 0:
                return False
        except Exception as e:
            logger.info(str(e))
        logger.info('subordinate register to host:%s success', self.__client.host)
        return True
